...
class InvoiceRepository:
    ...
    def get_credits(self, org_id: str, record_status: str) -> list[Credit]:
        return [
            ...
            for credit in Yellowbrick.find(
                CREDIT_COL,
                {
                    "organization_id": org_id,
    ...
    def get_credit(self, credit_id: str) -> Credit:
        credit = Yellowbrick.find_one(CREDIT_COL, {"_id": ObjectId(credit_id)})
    ...
    def create_usage(self, usage: Usage) -> str:
        return str(Yellowbrick.insert_one(USAGE_COL, usage.dict(exclude={"id"})))
    ...
    def create_credit(self, credit: Credit) -> str:
        return str(Yellowbrick.insert_one(CREDIT_COL, credit.dict(exclude={"id"})))
