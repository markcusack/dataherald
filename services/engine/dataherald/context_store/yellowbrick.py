...
class YellowbrickContextStore(DefaultContextStore):
    def __init__(self, system: System):
        super().__init__(system)

    def get_collection(self, db_connection_id:int) -> str:
        ...
        collection = parse_dsn(pg_uri).get("dbname")
        return self.golden_sql_collection + "_" + collection
    ...
    def retrieve_context_for_question(
        ...
        closest_questions = self.vector_store.query(
            ...
            collection=self.get_collection(prompt.db_connection_id),
    ...
    def add_golden_sqls(self, golden_sqls: List[GoldenSQLRequest]) -> List[GoldenSQL]:
        ...
        for record in golden_sqls:
            ...
            stored_collections.append(self.get_collection(golden_sql.db_connection_id))
        self.vector_store.add_records(stored_golden_sqls, stored_collections)
    ...
    def remove_golden_sqls(self, ids: List) -> bool:
        ...
        golden_sqls_repository = GoldenSQLRepository(self.db)
        for id in ids:
            golden_sql = golden_sqls_repository.find_by_id(id)
            collection = self.get_collection(golden_sql.db_connection_id)
            self.vector_store.delete_record(
                collection=collection, db_connection_id=golden_sql.db_connection_id, id=id
            )
